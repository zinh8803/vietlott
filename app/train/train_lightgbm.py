"""Train LightGBM classifier với RandomizedSearchCV + TimeSeriesSplit."""
...
def train_lightgbm(
    ...
    """Train LightGBM với RandomizedSearchCV."""
    try:
        from lightgbm import LGBMClassifier
        import lightgbm as lgb
    except ImportError:
        logger.error("lightgbm chưa được cài. Chạy: pip install lightgbm")
        return None

    logger.info(f"Training LightGBM cho {product_code}...")
    ...
    search = RandomizedSearchCV(
        ...
        cv=tscv,
        random_state=42,
        n_jobs=1,  # lightgbm tự dùng n_jobs nội bộ
        verbose=0,
    )

    # Dùng numpy array để tránh feature name warning của LightGBM
    search.fit(X_train.astype("float32"), y_train)
    ...
    logger.info(f"LightGBM: p@6={p6:.4f}, log_loss={ll:.4f}, brier={bs:.4f}")
    ...
    model_name = f"{product_code}_lightgbm_{feature_version}_w{window_size}"
    ...
    db_model = Model(
        ...
        framework_versions_json={"lightgbm": lgb.__version__, "python": platform.python_version()},
        status="challenger",
    )
    db.add(db_model)
    db.flush()
